...
# number of significant q < 0.05 values
n_sigq_0.05 <- length(sol$q_value[sol$q_value < 0.05])
```
...
The distribution of q values  and andhe abundance of `r n_sigq_0.01` proteins differed significantly between at least two cell lines. There were: `r row.names(sol_sigq_0.01)`

```{r q-fig, fig.width=6, fig.height=4,fig.cap="(ref:q-fig)"}
lab0.05 <- bquote(italic(n)~"="~.(n_sigq_0.05))
...
sol %>% ggplot(aes(q_value)) +
  ...
  annotate("text", x = 0.03, y = 2.4, label = expression(italic(p)~"< 0.05")) +
...
(ref:q-fig) q figure and some info about it
