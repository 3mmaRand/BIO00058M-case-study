...
```{r pkg}
source("../R/00-pkg.R", local = knitr::knit_global())
```

```{r functions}
# data_summary calculates summary statistics (mean, sample size, standard deviation,
# standard error) for a numeric variable. Used to summarise the protein abundances 
# for each cell line
source("../R/data_summary.R", local = knitr::knit_global())
```

```{r labels}
# creates two vectors, lineage and functionality which hold the labels for the cell types
# and their functional types.
# these are used in figures and for ordering factor levels 
# lineage is:
# "Y1015"
# "Y101"
# "Y201"
...
# "Immunomodulatory"
# "Immunomodulatory"
# note that the functionality labels match the appropriate cell type by position in the vector
source("../R/01-labels.R", local = knitr::knit_global())
...
The data are mass spectrometry data of the soluble protein fraction from the five immortalised mesenchymal stromal cell (MSC) lines.
...
The data in comparison_p\_and_q.csv give the p and q values for pairwise comparisons between cell lines.
...
## Data preparation {.unnumbered #methods-data-preparation}

```{r data-filter}
# Remove irrelevant rows. These are 1. bovine serum proteins which are from medium 
# on which the cells were grown and 2. Peptides with uncertainty identity which occurs when
# fewer than 2 peptides were detected for a protein. Keep only rows of human proteins 
# for which at least 2 peptides were identified
sol <- sol %>% 
...
# go to a column named lineage_rep which is then split into two separate columns
...
sol_summary_line <- data_summary(sol2, 
                            measure = abundance, 
                            group1 = line)
# capitalise the cell line and order the factor levels
...
# (which has a protein in each row)
# requires protein-line summary info to be spread so there is a protein per row
...
# we have p values for. This is all the pairwise comparisons
# except that y1015 is compared only to y101, no others
sol <- sol %>% 
...
## Visualisation with Principal Components Analysis {.unnumbered #methods-visualisation}
...
Principal Components Analysis was used for dimensionality-reduction to allow visualisation the distance between samples. It was conducted on protein abundances scaled to unit variance to prevent highly abundant proteins dominating the analysis. The package **`GGally`** [@R-GGally] was used to produced pairwise scatter plots of the first six principal components.

## Clustering and heatmaps {.unnumbered #methods-clustering-and-heatmaps}
...
(ref:protein-venn-fig) Protein set shared by cells lines. Created with package **`VennDiagram`** [@R-VennDiagram]
...
## Principal Components analysis {.unnumbered #results-principal-components-analysis}
...
(ref:pca-vis-fig) Samples represented on the first six Principal Components capturing `r percent6`% of the variation in soluble protein expression between samples. The leading diagonal shows the distribution of scores on each component with pairwise scatter plots of Principal Components below the diagonal. Created with the **`GGally`** [@R-GGally] and **`patchwork`** [@R-patchwork] packages.

<!-- end of fig legend -->


## Clustering and heatmaps {.unnumbered #results-cluster-heat}
...
The two immunomodulatory (see Figure \@ref(fig:heatmap-fig)) form one clus
...
## Volcano plots {.unnumbered #methods-volcano-plots}

incomplete. needs additional plots, formatting, legends
...
Word count calculated with **`wordcountaddin`** [@R-wordcountaddin]. The session information was written into a separate file rather than to the README this has been added to the wordcount.
