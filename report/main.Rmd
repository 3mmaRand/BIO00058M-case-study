...
df %>% 
  ...
  geom_col(width = 1) +
  geom_label(aes(x = x, y = y*2/3, label = lines),
             fill = "white",
             size = 3,
             label.size = 0,
             label.padding = unit(0.3, "lines")) +
  geom_label(aes(x = x, y = y*1/6, label = functionality),
             fill = "white",
             size = 2,
             label.size = 0,
             label.padding = unit(0.3, "lines")) +
...
## Visualisation {#methods-visualisation}
...
```{r cluster-prep}
# Matrix format
mat <- apply(sol_sigq_0.01, 1, z_score) %>% t()
...
```{=tex}
\begin{equation}
z_{ij} = \frac{x_{ij} - \bar{x_{i}}}{sd_{i}}  
(\#eq:zscore)
\end{equation}
```
where: $z_{ij}$ is
...
```{r pca-vis}
pca_labelled <- data.frame(pca$x, lineage = factor(toupper(tsol$lineage), levels = lines))
